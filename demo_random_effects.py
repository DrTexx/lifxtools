...
from lifxtools import return_interface, get_lights, list_lights, blink_light, ManagedLight, ManagedLifx # used for network discovery and control
...
mlifx.print_device_labels() # list all devices found on the network

for light in mlifx.lights:

    light_label = light.get_label()

    if (light_label == "Office Ceiling"):

        ceiling_light = light

mcl = ManagedLight(ceiling_light) # create a new managed light (supports saving/loading state and more)
mcl.ssave() # save the lights state
mcl.print_saved_state() # print the saved light state
...
try:
    ...
    if (enable_loop_rainbow == True):
        rainbow_loop_brightness = mcl.color[2]
        while True:
    ...
    try:
        fade_out_and_in(ceiling_light)
        mcl.sload() # load the lights state
    except:
        sleep(1)
